...
export function mockDiscoveryInvalidSchema(discoveryUrl: string) {
  server.use(http.get(discoveryUrl, async () => HttpResponse.json({ wrong: 'shape' })));
}
...
export function mockDiscoveryHttpError(
  ...
  server.use(http.get(discoveryUrl, async () => HttpResponse.text(body, { status })));
}
    ...
    http.get(userinfoUrl, async ({ request }) => {
      if (expectedHeaders) {
        for (const [key, value] of Object.entries(expectedHeaders)) {
          ...
          if (header !== value) {
            return HttpResponse.text(`Unexpected header ${key}: ${header} !== ${value}`, {
              status: 400,
            });
          }
        }
...
export function mockUserinfoInvalidSchema(userinfoUrl: string) {
  server.use(http.get(userinfoUrl, async () => HttpResponse.json({ wrong: 'shape' })));
}
...
export function mockUserinfoHttpError(
  ...
  server.use(http.get(userinfoUrl, async () => HttpResponse.text(body, { status })));
}
